...
> [!NOTE]  
> If you don't need a detailed explanation, you can follow the checklist from
...
> [!NOTE]  
> If you are using a pre-release version of `PleOps.Cake`, you will need to add
> a `nuget.config` to define the Azure DevOps NuGet feed. Create the file in the
> directory where your `.csproj` is with the following content.
>
> ```xml
...
Now let's define the content of our program. Our program will run one command:
create a `CakeHost`, configure assemblies with tasks and run with the
...
your project. Keep reading to configure it to your paths.

## Configure .NET projects

TODO: dependencies, build props, ispackable, define in build system
